...
public class InfoController {
    ...
    // 루트 페이지 경로
    @RequestMapping("/")
    public String root() throws Exception { return "redirect:main"; }
    ...
    public String mainpro(Model model, Pageable pageable) {
        if (pageable.getSort().toString() == "UNSORTED") {
            ...
            model.addAttribute("projectList2", projectService.getProjectList("종료", "전체", pageable));
            // 예정
    ...
    public ResponseEntity<List<ProjectDto>> keywordSearch(@PathVariable(name = "keyword") String keyword, Model model) {
        return new ResponseEntity<>(projectService.findAllByProTitleLike(keyword), HttpStatus.OK);
    }
