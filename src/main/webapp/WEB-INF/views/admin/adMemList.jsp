...
<script>
  ...
  function openMemberDetail(memNo) {
    ...
    $(".member-detail-back").css({
      "width" : $(document).width(),
      "height" : $(document).height()
    });
    $(".member-detail-back").fadeTo("slow", 0.8);
    $(".member-detail").fadeTo("slow", 1);
  }

  window.onload = function() {
    // 열기 (MORE 클릭)
    const openMembers = document.querySelectorAll(".open-member");
    for (const openMember of openMembers) {
      openMember.onclick = function(e) {
        openMemberDetail($(this).data("mem-no"));
      };
    }
    
    // 닫기 (배경 클릭)
    document.querySelector(".member-detail-back").onclick = function(e) {
      this.style.display = "none";
      document.querySelector(".member-detail").style.display = "none";
    };
    
    // 정렬 관련
    document.querySelector(".member-order").onclick = function(e) {
      $(location).attr("href", $(this).attr("href"));
    };

    let orderValue1 = "";
    $.each(document.querySelector("#headerSortParam").value.split(","),
      function(index, item) {
        const param1 = $.trim(item.split(":")[0]);
        const direction1 = $.trim(item.split(":")[1].toLowerCase());
        if (param1 == "memNo") {
    ...
    $(".member-order").each(
      function(index, item) {
        let orderValue2 = "";
        const param2 = $(this).data("param");
        const direction2 = $(this).data("direction").toLowerCase();
        // 경로에서 기존 정렬 문구 삭제
        $(this).empty();
        ...
        $(this).attr(
          "href",
          "/admin/memberList?page="
            + document.querySelector("#headerPageParam").value
            + orderValue2);
      });
...
<div class="container">
  ...
  <div class="row">
    ...
    <div class="col text-end align-self-end">
      <span>가입 ${todayCount}명</span> <span>| 총 ${list.totalElements}명</span>
    </div>
  </div>
  <hr>
  ...
  <!-- 문의 -->
  <div>
    <input type="hidden" id="headerSortParam" value="${list.sort}"> <input type="hidden"
      id="headerPageParam" value="${list.number}">
    <table class="table table-striped table-hover">
      <tr>
        <th style="width: 10%;">no</th>
        <th style="width: 20%;"><a href="#" class="member-order memId" data-param="memId"
          data-title="아이디" data-direction=""></a></th>
        <th style="width: 10%;"><a href="#" class="member-order memName" data-param="memName"
          data-title="이름" data-direction=""></a></th>
        <th style="width: 15%;"><a href="#" class="member-order memJoinDate"
          data-param="memJoinDate" data-title="가입일" data-direction=""></a></th>
      ...
      <c:forEach items="${list.content}" var="memberDto" varStatus="status">
        <tr>
          ...
          <td><a href="#" class="open-member" data-mem-no="${memberDto.memNo}"> <strong><em>MORE</em></strong>
          </a></td>
        </tr>
      </c:forEach>
    </table>
  ...
  <div class="text-center">
    <ul class="pagination">
      ...
      <c:forEach var="i" begin="${pageIndex * pageRange + 1}" end="${lastPage}">
        <c:choose>
          <c:when test="${list.number + 1 == i}">
            <li class="page-item active"><a href="#" class="page-link"> <c:out value="${i}" />
            </a></li>
          </c:when>
          <c:otherwise>
            <li class="page-item"><a href="?page=${i - 1}" class="page-link"> <c:out
                  value="${i}" />
            </a></li>
          </c:otherwise>
        </c:choose>
      ...
      <c:if test="${!list.last}">
        <li class="page-item"><a href="?page=${list.number + 1}" class="page-link">&gt;</a></li>
        <li class="page-item"><a href="?page=${list.totalPages - 1}" class="page-link">&raquo;</a>
        </li>
      </c:if>
    </ul>
  </div>
</div>
<div class="member-detail-back"></div>
