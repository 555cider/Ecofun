					...
					<ul class="pagination justify-center" style="justify-content: center;">
						...
						<c:forEach var="i" begin="${pageIndex * pageRange + 1}" end="${lastPage}">
							<c:choose>
								...
								<c:otherwise>
									<li class="page-item">
										<a href="?sort=askNo,desc&page=${i}" class="page-link"><c:out value="${i}" /></a>
									</li>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						...
						<c:forEach var="k" begin="${pageIndex2 * pageRange2 + 1}" end="${lastPage2}">
							<c:choose>
								<c:when test="${aplList.number + 1 == k}">
									<li class="page-item active">
										<a href="#" class="page-link"><c:out value="${k}" /></a>
									</li>
								</c:when>
								<c:otherwise>
									<li class="page-item">
										<a href="?sort=aplNo,desc&page=${k - 1}" class="page-link"><c:out value="${k}" /></a>
									</li>
								</c:otherwise>
							</c:choose>
						</c:forEach>
