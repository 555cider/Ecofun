...
<script>
  ...
  $(function() {
    ...
    $(".pro-state .dropdown-item").click(function() {
      const proType = $(".pro-type a.active").html();
      const proState = $(this).html();
      let getParam = "";
      if (proType != "전체") {
    ...
    document.querySelector(".pro-type .dropdown-item").onclick = function() {
      const proState = $(".pro-state a.active").html();
      const proType = $(this).html();
      let getParam = "";
      if (proState != "전체") {
        getParam += "&proState=" + proState;
      }
    ...
    document.querySelector(".deleteLike").onclick = function() {
...
<div class="container">
  ...
  <div class="d-flex">
    ...
    <div class="col">
      <div class="btn-group">
        <button class="btn btn-light dropdown-toggle pro-state" type="button" data-toggle="dropdown"
          aria-haspopup="true" aria-expanded="false">상태(전체)</button>
        <div class="dropdown-menu pro-state" style="min-width: auto;">
          <a class="dropdown-item active" href="#">전체</a>
          <div class="dropdown-divider"></div>
          <a class="dropdown-item" href="#">진행</a> <a class="dropdown-item" href="#">종료</a> <a
            class="dropdown-item" href="#">예정</a>
        </div>
      </div>
      <div class="btn-group">
        <button class="btn btn-light dropdown-toggle pro-type" type="button" data-toggle="dropdown"
          aria-haspopup="true" aria-expanded="false">분류(전체)</button>
  ...
  <ul class="row">
    <c:forEach items="${likeList.content}" var="likeDto" varStatus="like">
      <li class="col-sm-6 col-lg-4 project-item"><a
        href="/project/detail?proNo=${likeDto.proNo}"> <img src="${likeDto.projectDto.proThumb}"
          alt="이미지" style="width: 100%;"> <span class="progress-bar" role="progressbar"
          style="width: ${likeDto.projectDto.proceed}%;"
          aria-valuenow="${likeDto.projectDto.proceed}" aria-valuemin="0" aria-valuemax="100">${likeDto.projectDto.proceed}%</span>
          <span class="project-item-title">${likeDto.projectDto.proTitle}</span> <br>
          ${likeDto.projectDto.proStart} ~ ${likeDto.projectDto.proEnd} <br>
      </a> <input type="button" class="deleteLike" data-pro-no="${likeDto.proNo}" value="해제하기">
      </li>
    </c:forEach>
  </ul>
  <br>
  ...
  <div class="text-center">
    <ul class="pagination">
      <c:if test="${!likeList.first}">
        ...
        <li class="page-item"><a href="?page=${likeList.number - 1}" class="page-link">&lt;</a>
        </li>
      </c:if>
      ...
      <c:if test="${!likeList.last}">
        ...
        <li class="page-item"><a href="?page=${likeList.totalPages - 1}" class="page-link">&raquo;</a>
        </li>
      </c:if>
    </ul>
  </div>
