...
<script>
  ...
  $(
    function() {
      if (proType != "") {
        $("input[name='projectType']:radio[value='" + proType + "']").prop(
          "checked", true);
      ...
      $(".page-item:not(.active) a").each(
        function(index, item) {
          $(this).attr("href",
            appendBefore + $(this).attr("href") + appendAfter);
        });
...
<div class="container">
  ...
  <div class="col-10 text-center content-center">
    <table class="table-bordered">
      <tr>
        <th class="col-6">
          <h5>총 참여 금액</h5> <fmt:formatNumber value="${sum}" pattern="#,##0" /> 원
        </th>
        <th class="col-6">
          <h5>총 참여 횟수</h5> <c:out value="${orderList.totalElements}" /> 회
        </th>
      </tr>
      ...
      <tr>
        <td colspan="2">
          <form action="/mypage/projectList" method="get" onsubmit="return formCheck(this)">
            <input type="radio" name="projectType" id="" value=""> <label for="all">전체</label>
            <input type="radio" name="projectType" id="don" value="기부"> <label for="don">기부</label>
            <input type="radio" name="projectType" id="fun" value="펀딩"> <label for="fun">펀딩
            </label> <br> <input type="date" name="startDate" id="startDate"> ~ <input
              type="date" name="endDate" id="endDate"> <input type="submit" value="조회">
          </form>
        </td>
      </tr>
      <tr>
        <td colspan="2"><input type="button" class="date-button" data-range="1" value="1개월">
          <input type="button" class="date-button" data-range="3" value="3개월"> <input
          type="button" class="date-button" data-range="6" value="6개월"></td>
      </tr>
    </table>
  </div>
  <br>
    ...
    <c:forEach items="${orderList.content}" var="ordersDto" varStatus="order">
      <div class="row col-12 content-center">
        ...
        <div class="col-10 col-lg-11 d-flex">
          <img src="${projectList[order.index].proThumb}" alt="이미지" class="content-center"
  ...
  <div class="text-center">
    <ul class="pagination">
      <c:if test="${!orderList.first}">
        ...
        <li class="page-item"><a href="?page=${orderList.number - 1}" class="page-link">&lt;</a>
        </li>
      </c:if>
      ...
      <c:if test="${!orderList.last}">
        ...
        <li class="page-item"><a href="?page=${orderList.totalPages - 1}" class="page-link">&raquo;</a>
        </li>
      </c:if>
    </ul>
  </div>
