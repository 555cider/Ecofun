  ...
  <div class="row">
    ...
    <div class="col">
      <ul class="nav nav-tabs">
        <li class="nav-item"><a class="nav-link active" data-toggle="tab" href="#ask">문의</a></li>
        <li class="nav-item"><a class="nav-link" data-toggle="tab" href="#apply">신청</a></li>
      </ul>
    </div>
    <div class="col text-end">
      <span>문의: ${askCount}건</span> <span>| 신청: ${aplCount}건</span>
    </div>
  </div>
  <br>
      ...
      <div class="text-center">
        <ul class="pagination">
          <c:if test="${!askList.first}">
            <li class="page-item"><a href="?sort=askNo,desc$page=" class="page-link">&laquo;</a>
            </li>
            <li class="page-item"><a href="?sort=askNo,desc&page=${askList.number - 1}"
          ...
          <c:forEach var="i" begin="${pageIndex * pageRange + 1}" end="${lastPage}">
            <c:choose>
              ...
              <c:otherwise>
                <li class="page-item"><a href="?sort=askNo,desc&page=${i}" class="page-link">
                    <c:out value="${i}" />
                </a></li>
              </c:otherwise>
            </c:choose>
          ...
          <c:if test="${!askList.last}">
            <li class="page-item"><a href="?sort=askNo,desc&page=${askList.number + 1}"
              class="page-link">&gt;</a></li>
            <li class="page-item"><a href="?sort=askNo,desc&page=${askList.totalPages - 1}"
    ...
    <div class="tab-pane fade show" id="apply">
      <div>
        <table class="table text-center">
          ...
          <c:forEach var="req" items="${aplList.content}">
            <fmt:parseDate value="${apl.aplDate}" var="dateValue2" pattern="yyyy-MM-dd'T'HH:mm" />
            <tr>
              <td>${apl.aplNo}</td>
              <td><a href="/mypage/myAplDetail?aplNo=${apl.aplNo}">${apl.aplTitle}</a></td>
              <td><fmt:formatDate value="${dateValue2}" pattern="yyyy-MM-dd" /></td>
              <td>검토중</td>
            </tr>
          </c:forEach>
        </table>
      ...
      <div class="text-center">
        <ul class="pagination">
          <c:if test="${!aplList.first}">
            <li class="page-item"><a href="?sort=aplNo,desc" class="page-link">&laquo;</a></li>
            <li class="page-item"><a href="?sort=aplNo,desc&page=${aplList.number - 1}"
              class="page-link">&lt;</a></li>
          </c:if>
          <c:set var="pageRange2" scope="session" value="5" />
          <fmt:parseNumber var="pageIndex2" integerOnly="true"
            value="${aplList.number div pageRange2}" />
          <c:choose>
          ...
          <c:forEach var="k" begin="${pageIndex2 * pageRange2 + 1}" end="${lastPage2}">
            <c:choose>
              ...
              <c:otherwise>
                <li class="page-item"><a href="?sort=aplNo,desc&page=${k - 1}"
                  class="page-link"> <c:out value="${k}" />
                </a></li>
              </c:otherwise>
            </c:choose>
          </c:forEach>
          <c:if test="${!aplList.last}">
            <li class="page-item"><a href="?sort=aplNo,desc&page=${aplList.number + 1}"
              class="page-link">&gt;</a></li>
            <li class="page-item"><a href="?sort=aplNo,desc&page=${aplList.totalPages - 1}"
              class="page-link">&raquo;</a></li>
          </c:if>
        </ul>
      </div>
